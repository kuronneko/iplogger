...
<template>
    ...
    <div v-if="canLogin" class="absolute top-0 right-0 px-6 py-4 sm:block">
        <Link v-if="$page.props.user" :href="route('dashboard')"
            class="text-md text-gray-700 dark:text-gray-500">Dashboard</Link>
        ...
        <template v-else>
            ...
            <Link v-if="canRegister" :href="route('register')"
                class="ml-4 text-md text-gray-700 dark:text-gray-500">Register</Link>
        </template>
    </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <div class="p-6 sm:px-20">
                ...
                <div class="mt-6 text-gray-500">
                    Kuro.IPLogger! is a web service that provides the possibility to create an account and generate an
                    URL to capture IPs.
                    <br>
            ...
            <div class="flex justify-center mt-6">
                <a
                    ...
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Your Network Properties</h5>
                        <div class="font-normal text-gray-700 dark:text-gray-400">
                            <p><span class="font-bold">Date/Time: </span>{{ moment(date).format('MMMM Do YYYY, h:mm:ss a') }}</p>
                            <p><span class="font-bold">IP Address: </span>{{ location.ip }}</p>
                            <p><span class="font-bold">Country: </span>{{ location.countryName }}, {{ location.cityName }}</p>
            ...
            <div class="flex justify-center mt-2 text-gray-500 p-6 sm:px-20">
                <Link v-if="canRegister" :href="route('register')"
                class="ml-4 font-bold">Sing Up</Link> <span class="ml-1">and start using our services!</span>
            </div>
